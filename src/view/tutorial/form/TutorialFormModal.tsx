...
import TutorialForm from 'src/view/tutorial/form/TutorialForm';
import TutorialService from 'src/modules/tutorial/tutorialService';

function TutorialFormModal(props) {
  ...
  const doSubmit = async (_, data) => {
    try {
      setSaveLoading(true);
      const { id } = await TutorialService.create(data);
        ...
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">
              {i18n('entities.tutorial.new.title')}
            </h5>
            <button
              type="button"
              className="close"
