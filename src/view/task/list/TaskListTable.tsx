          ...
          <thead className="thead">
            <tr>
              <TableColumnHeader className="th-checkbox">
                ...
                <TableColumnHeader
                  label={i18n(
                    'entities.task.fields.taskBelongsToStation',
                  )}
                />
                <TableColumnHeader
                  onSort={doChangeSort}
                ...
                <TableColumnHeader
                  ...
                  name={'wasItDone'}
                  label={i18n(
                    'entities.task.fields.wasItDone',
                  )}
                />
                <TableColumnHeader
                  label={i18n(
                    'entities.task.fields.imageOptional',
                  )}
                />
                <TableColumnHeader
                  onSort={doChangeSort}
                ...
                <TableColumnHeader
                  label={i18n(
                    'entities.task.fields.taskCompletedImage',
                  )}
                />
            ...
            {!loading &&
              rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td>
                    <StationListItem value={row.taskBelongsToStation} />
                  </td>
                  <td>{row.taskToDo}</td>
                  <td>
                    {row.wasItDone
                  ...
                  <td>
                    <ImagesListView value={row.imageOptional} />
                  </td>
                  <td>
                    {row.dateToDoTheTask
                      ? moment(row.dateToDoTheTask).format(
                          'YYYY-MM-DD HH:mm',
                        )
                      : null}
                  </td>
                  <td>
                    {row.dateCompletedTask
                      ? moment(row.dateCompletedTask).format(
                          'YYYY-MM-DD HH:mm',
                        )
                      : null}
                  </td>
                  <td>
                    <ImagesListView value={row.taskCompletedImage} />
                  </td>
