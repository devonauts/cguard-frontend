import React from 'react';
import { i18n } from 'src/i18n';
import actions from 'src/modules/inventory/importer/inventoryImporterActions';
import fields from 'src/modules/inventory/importer/inventoryImporterFields';
import selectors from 'src/modules/inventory/importer/inventoryImporterSelectors';
import ContentWrapper from 'src/view/layout/styles/ContentWrapper';
import Breadcrumb from 'src/view/shared/Breadcrumb';
import importerHoc from 'src/view/shared/importer/Importer';
import PageTitle from 'src/view/shared/styles/PageTitle';

function InventoryImportPage() {
  const Importer = importerHoc(
    selectors,
    actions,
    fields,
    i18n('entities.inventory.importer.hint'),
  );

  return (
    <>
      <Breadcrumb
        items={[
          [i18n('dashboard.menu'), '/'],
          [i18n('entities.inventory.menu'), '/inventory'],
          [i18n('entities.inventory.importer.title')],
        ]}
      />

      <ContentWrapper>
        <PageTitle>
          {i18n('entities.inventory.importer.title')}
        </PageTitle>

        <Importer />
      </ContentWrapper>
    </>
  );
}

export default InventoryImportPage;
