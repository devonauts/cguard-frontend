...
import StationForm from 'src/view/station/form/StationForm';
import StationService from 'src/modules/station/stationService';

function StationFormModal(props) {
  ...
  const doSubmit = async (_, data) => {
    try {
      setSaveLoading(true);
      const { id } = await StationService.create(data);
        ...
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">
              {i18n('entities.station.new.title')}
            </h5>
            <button
              type="button"
              className="close"
