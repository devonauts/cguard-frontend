...
import VideoTutorialListItem from 'src/view/videoTutorial/list/VideoTutorialListItem';

function VideoTutorialCategoryListTable(props) {
  const [
    recordIdToDestroy,
  ...
  const hasPermissionToEdit = useSelector(
    videoTutorialCategorySelectors.selectPermissionToEdit,
  );
            ...
            <tr>
              <TableColumnHeader className="th-checkbox">
                ...
                <TableColumnHeader
                  ...
                  name={'categoryName'}
                  label={i18n(
                    'entities.videoTutorialCategory.fields.categoryName',
                  )}
                />
                <TableColumnHeader
                  label={i18n(
                    'entities.videoTutorialCategory.fields.videosInCategory',
                  )}
                />
            ...
            {!loading &&
              rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td>
                    <VideoTutorialListItem value={row.videosInCategory} />
                  </td>
                  <td className="td-actions">
                    <Link
                      className="btn btn-link"
                      to={`/video-tutorial-category/${row.id}`}
                    >
                      {i18n('common.view')}
                    </Link>
                    {hasPermissionToEdit && (
                      <Link
                        className="btn btn-link"
                        to={`/video-tutorial-category/${row.id}/edit`}
                      >
                        {i18n('common.edit')}
                      </Link>
                    )}
...
export default VideoTutorialCategoryListTable;
