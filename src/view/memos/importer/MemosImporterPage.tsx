import React from 'react';
import { i18n } from 'src/i18n';
import actions from 'src/modules/memos/importer/memosImporterActions';
import fields from 'src/modules/memos/importer/memosImporterFields';
import selectors from 'src/modules/memos/importer/memosImporterSelectors';
import ContentWrapper from 'src/view/layout/styles/ContentWrapper';
import Breadcrumb from 'src/view/shared/Breadcrumb';
import importerHoc from 'src/view/shared/importer/Importer';
import PageTitle from 'src/view/shared/styles/PageTitle';

function MemosImportPage() {
  const Importer = importerHoc(
    selectors,
    actions,
    fields,
    i18n('entities.memos.importer.hint'),
  );

  return (
    <>
      <Breadcrumb
        items={[
          [i18n('dashboard.menu'), '/'],
          [i18n('entities.memos.menu'), '/memos'],
          [i18n('entities.memos.importer.title')],
        ]}
      />

      <ContentWrapper>
        <PageTitle>
          {i18n('entities.memos.importer.title')}
        </PageTitle>

        <Importer />
      </ContentWrapper>
    </>
  );
}

export default MemosImportPage;
