...
function PatrolCheckpointListTable(props) {
  const [
    recordIdToDestroy,
  ...
  const hasPermissionToEdit = useSelector(
    patrolCheckpointSelectors.selectPermissionToEdit,
  );
          ...
          <thead className="thead">
            <tr>
              <TableColumnHeader className="th-checkbox">
                ...
                <TableColumnHeader
                  label={i18n(
                    'entities.patrolCheckpoint.fields.station',
                  )}
                />
                <TableColumnHeader
                  onSort={doChangeSort}
                  ...
                  name={'name'}
                  label={i18n(
                    'entities.patrolCheckpoint.fields.name',
                  )}
                />
                <TableColumnHeader
                  onSort={doChangeSort}
                  ...
                  name={'latitud'}
                  label={i18n(
                    'entities.patrolCheckpoint.fields.latitud',
                  )}
                />
                <TableColumnHeader
                  onSort={doChangeSort}
                  ...
                  name={'longitud'}
                  label={i18n(
                    'entities.patrolCheckpoint.fields.longitud',
                  )}
                />
                <TableColumnHeader
                  label={i18n(
                    'entities.patrolCheckpoint.fields.assignedQrImage',
                  )}
                />
                <TableColumnHeader
                  label={i18n(
                    'entities.patrolCheckpoint.fields.patrols',
                  )}
                />
            ...
            {!loading &&
              rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td>{row.longitud}</td>
                  <td>
                    <ImagesListView value={row.assignedQrImage} />
                  </td>
                  <td>
                    <PatrolListItem value={row.patrols} />
                  </td>
                  <td className="td-actions">
                    <Link
                      className="btn btn-link"
                      to={`/patrol-checkpoint/${row.id}`}
                    >
                      {i18n('common.view')}
                    </Link>
                    {hasPermissionToEdit && (
                      <Link
                        className="btn btn-link"
                        to={`/patrol-checkpoint/${row.id}/edit`}
                      >
                        {i18n('common.edit')}
                      </Link>
                    )}
...
export default PatrolCheckpointListTable;
